...
merge_summary <- rbind(pleiades, m67)


```

The 9 features we are looking at 
...
pleiades_corr <- ggplot(data = melted_cormat_pleiades, aes(Var2, Var1, fill = value))+
 ...
 theme(axis.text.x = element_text(angle = 45, vjust = 1, 
    size = 12, hjust = 1))+
 coord_fixed()+
  xlab("Variable 2") + ylab("Variable 1") + ggtitle("Correlation of Pleiades Features") + geom_tile() +
...
#correlation of m67
```{r}
m67corr <- na.omit(m67) #removed missing values
cormatm67 <- round(cor(m67corr),2)
pandoc.table(cormatm67)
```

```{r}
melted_cormat_m67 <- melt(cormatm67)
m67_corr <- ggplot(data = melted_cormat_m67, aes(Var2, Var1, fill = value)) +
 geom_tile(color = "white") +
 scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
   midpoint = 0, limit = c(-1,1), space = "Lab", 
   name="Pearson\nCorrelation\nof\nM67\nFeatures") +
 ...
 theme(axis.text.x = element_text(angle = 45, vjust = 1, 
    size = 12, hjust = 1))+
 coord_fixed()+
  xlab("Variable 2") + ylab("Variable 1") + ggtitle("Correlation of M67 Features")+geom_tile() +
geom_text(aes(Var2, Var1, label = value),
          color = "black", size = 2)
```
...
ggplot(data = melted_cormat, aes(Var2, Var1, fill = value)) +
 ...
 theme(axis.text.x = element_text(angle = 45, vjust = 1, 
    size = 12, hjust = 1))+
 coord_fixed()+
  xlab("Variable 2") + ylab("Variable 1") + ggtitle("Correlation of Features")+geom_tile() +
geom_text(aes(Var2, Var1, label = value),
          color = "black", size = 2)
...
#ggplot(merge, aes(x = class, y = abM)) + geom_boxplot()
#ggplot(merge, aes(x = class, y = parallax)) + geom_boxplot()
```
...
From this diagram we are able to estimate the age of the clusters (reference). Pleiades is a young open cluster, meaning it has a lot of its stars on the 'main sequence'. On the other hand, m67 is a much older cluster, meaning the stars near the upper ends appear to tail off . Note that this is outside the scope of the EDA, but is useful to show to guide future analysis.
