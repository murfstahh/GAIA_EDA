...
## comparison of some means of variables
...
pleiades_corr <- ggplot(data = melted_cormat_pleiades, aes(Var2, Var1, fill = value))+
 ...
 theme(axis.text.x = element_text(angle = 45, vjust = 1, 
    size = 12, hjust = 1))+
 coord_fixed()+
  xlab("Variable 2") + ylab("Variable 1") + ggtitle("Correlation of Pleiades Features")
...
m67_corr <- ggplot(data = melted_cormat_m67, aes(Var2, Var1, fill = value)) +
 ...
 theme(axis.text.x = element_text(angle = 45, vjust = 1, 
    size = 12, hjust = 1))+
 coord_fixed()+
  xlab("Variable 2") + ylab("Variable 1") + ggtitle("Correlation of M67 Features")
```
...
d <- ggplot(merge, aes(x = abM)) + geom_histogram(aes(fill= class), position = "dodge") + xlab("Absolute Magnitude (mag)") + ylab("Count") 
t <- theme(plot.title = element_text(face="bold"))
c + d + plot_annotation(title = "Appendix X - Parallax and Absolute Magnitude of Pleiades and m67", theme = t) 
```
...
From this diagram we are able to estimate the age of the clusters. Pleiades is a young open cluster, meaning it has a lot of its stars on the 'main sequence'. On the other hand, m67 is a much older cluster, meaning the stars near the upper ends appear to tail off.
